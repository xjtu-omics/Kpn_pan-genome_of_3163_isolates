...
# Extract feature columns and phenotypes from the previously split validation dataset to build and save the validation matrix
...
for med in os.listdir(fm_root):
    ...
    if os.path.isdir(path):
        ...
        if not os.path.exists(pkl_path):
            print(f"No model: {pkl_path}")
            continue
        with open(pkl_path, "rb") as f:
            model = joblib.load(f)

        # Read the training feature columns (including phenotype)
        ...
        # Extract the corresponding feature columns and phenotype from the full validation dataset
        ...
        val_total_data = pd.concat([val_core_data, val_dis_data], axis=1) # Merge
        val_total_data = val_total_data.loc[:, ~val_total_data.columns.duplicated()] # Deduplicate columns (label)
        val_total_data = val_total_data[feature_cols + ["label"]]
        val_total_data.to_csv(f"{val_fm_save_root}/{med}.csv")

        # Predict on the validation set and compute AUC. Write AUC values for all antibiotics to the same txt file.
